<template>
  <div class="max-w-lg flex items-center justify-start flex-col m-auto">
    <div v-if="plans.length">
      <h1 class="text-blue font-bold text-center text-4xl py-8">Open a Budget</h1>
      <div v-for="plan in plans" v-bind:key="plan.id" class="max-w-xs w-full">
        <a class="block no-underline" :href="`/budgets/${plan.id}`">
          <div class="bg-white shadow-md rounded px-8 py-6 mb-4 border-r-8 border-grey-light hover:border-green">
            <div class="font-bold text-grey-darkest text-2xl">{{ plan.name }}</div>
          </div>
        </a>
      </div>
    </div>
    <div v-else>
      <h1 class="text-blue font-bold text-center text-4xl py-8">Create a Budget</h1>
